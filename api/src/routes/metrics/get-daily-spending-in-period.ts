...
export const getDailySpendingInPeriodRoute: FastifyPluginAsyncZod =
  async app => {
    app.register(auth).get(
      "/metrics/daily-spending-in-period",
      {
        schema: {
          tags: ["metrics"],
          summary: "List daily spending for period",
      ...
      async (request, reply) => {
        ...
        const spendingPerDay = await db
          .select({
            spending: sum(transactions.value).mapWith(Number),
          ...
          .having(({ spending }) => gte(spending, 1))
        ...
        for (const value of eachDayOfInterval(interval)) {
          const exists = spendingPerDay.find(
        ...
        const spendings = spendingPerDay.sort((a, b) => {
        ...
        return reply.status(200).send(spendings)
      },
    )
  }
