...
describe("List monthly amount transactions [GET] /metrics/month-amount-transactions", () => {
  beforeAll(async () => {
    await app.ready()
  ...
  it("should be able to list monthly amount transactions", async () => {
    const response = await request(app.server)
      .get("/metrics/month-amount-transactions")
      .set("Cookie", token)
      .send()
    ...
    expect(response.body).toEqual(
      expect.objectContaining({
        monthWithYear: expect.any(String),
        amount: 20,
      }),
    )
  })
